...
`src/crevasse/nisar/` is deliberately flat within itself (no sub-packages) -- a further
hierarchy there would buy nothing. The repo as a whole IS a real installed package now
(`pip install -e .`, see the top-level README) -- no `sys.path.insert` needed; import as
`from crevasse.nisar.gate_predict import NisarGate` etc.
...
| 5 | `python src/crevasse/nisar/check_granule.py ab 025_048 025_019` | 359 tiles, 0.904 vs **0.627** |
| 5b | `python src/crevasse/nisar/check_granule.py ab 025_091 025_019` (control) | both arms near 0.9, delta ≈ +0.004 — a run that fails this is measuring the harness, not the granule |
