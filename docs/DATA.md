...
|---|---|---|
| `data/gate_5m_freqA_2gran.joblib` | 12 MB | **inference** — this is the model |
| `data/tile_labels_ongrid_5m.csv` | 15 KB | retraining (hand labels, 025_091) |
| `data/tile_labels_aoi_025_019_speedmatched.csv` | 100 KB | retraining |
| `data/tile_labels_aoi_025_091_speedmatched.csv` | 76 KB | retraining |
| `data/tile_labels_aoi_025_048_speedmatched.csv` | 64 KB | **reproducing the 025_048 failure only** — not training |
| `data/context_grid_2560m.npz` | 7 MB | rebuilding labels; continent-wide ice type + speed on the 2560 m tile grid |
| `data/_context_masks_025_*_t512.npz` | 145 KB each | `--grounded-only` scoring of those three granules |

Total ~19 MB. Nothing else is required to run inference.
...
Either drop them in `data/nisar/`, or leave them wherever they are and point at that tree:

```bash
export GATE_ROOT=/mnt/big/nisar-gate   # must contain data/nisar/*.tif
```

`GATE_ROOT` overrides the repo root for *all* data paths, so a tree used this way needs
`data/gate_5m_freqA_2gran.joblib` too (symlink the shipped one).
