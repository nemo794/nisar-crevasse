...
Data is resolved relative to the repo root. Set GATE_ROOT to point at a directory tree
holding data/nisar/ elsewhere -- the 48 GB of granules usually will not live in the repo.
"""
...
ROOT = Path(os.environ.get("GATE_ROOT", Path(__file__).resolve().parent.parent))
...
def find_granule(granule_id):
    ...
    for p in sorted((ROOT / "data" / "nisar").glob("*.tif")):
        ...
        if m and m.group(1) == granule_id:
            return p
    raise SystemExit(f"granule {granule_id} not found under {ROOT / 'data' / 'nisar'}/")
